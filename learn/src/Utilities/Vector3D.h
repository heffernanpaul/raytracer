...
class Vector3D {
	...
	public:
		...
		Vector3D(float a);										// constructor
		Vector3D(float _x, float _y, float _z);				// constructor
		...
		Vector3D												// multiplication by a float on the right
		operator* (const float a) const;
				
		Vector3D												// division by a float
		operator/ (const float a) const;
