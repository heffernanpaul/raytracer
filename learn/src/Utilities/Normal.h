...
class Normal {	
	...
	public:
		...
		Normal(float a);									// constructor
		Normal(float _x, float _y, float _z);			// constructor
		...
		Normal 												// multiplication by a float on the right
		operator* (const float a) const;
				
		void 												// convert normal to a unit normal
		normalize(void); 	

		friend std::ostream& operator<< (std::ostream& stream, const Normal& v){
