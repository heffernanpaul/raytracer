...
RGBColor
PerfectSpecular::f(ShadeRec& sr, const Vector3D& wo, const Vector3D& wi) const {
	return (black);
}
...
RGBColor
PerfectSpecular::sample_f(ShadeRec& sr, const Vector3D& wo, Vector3D& wi) const {
	float ndotwo = sr.normal * wo;
	wi = -(-wo + 2.0 * sr.normal * ndotwo); // wi is pointing into the surface
...
RGBColor
PerfectSpecular::sample_f(ShadeRec& sr, const Vector3D& wo, Vector3D& wi, float& pdf) const {
	float ndotwo = sr.normal * wo;
...
RGBColor
PerfectSpecular::rho(ShadeRec& sr, const Vector3D& wo) const {
	return (black);
}
