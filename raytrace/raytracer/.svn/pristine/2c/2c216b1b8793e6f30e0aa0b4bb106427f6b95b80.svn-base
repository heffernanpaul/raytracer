...
RGBColor
GlossySpecular::f(ShadeRec& sr, const Vector3D& wo, const Vector3D& wi) const {
	RGBColor 	L;  				
...
RGBColor
GlossySpecular::sample_f(ShadeRec& sr, const Vector3D& wo, Vector3D& wi, float& pdf) const {
...
RGBColor
GlossySpecular::rho(ShadeRec& sr, const Vector3D& wo) const {
	return (black);
}
