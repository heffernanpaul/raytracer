...
Reflective::shade(ShadeRec& sr) {	
	...
	L += fr * sr.w.render_ptr->tracer_ptr->trace_ray(reflected_ray, sr.depth + 1) * (sr.normal * wi);
...
Reflective::area_light_shade(ShadeRec& sr) {
    ...
    L += fr * sr.w.render_ptr->tracer_ptr->trace_ray(reflected_ray, sr.depth + 1) * (sr.normal * wi) / pdf;
