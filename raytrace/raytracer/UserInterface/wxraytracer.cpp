...
void RenderCanvas::OnRenderCompleted( wxCommandEvent& event )
{
   ...
   if(timer != NULL)
   {
      ...
      wxString timeString = timeElapsed.Format(wxT("Elapsed Time: %H:%M:%S.%l"));
...
{
   //set up float buffered device context
   wxClientDC cdc(this);
   DoPrepareDC(cdc);
...
void RenderCanvas::OnTimerUpdate( wxTimerEvent& event )
{
   ...
   long msecRemain = (long)
                       (((float)interval / ((0.01+completed)*100)) * 100 * remaining);
...
void RenderCanvas::renderStart(void)
{
   ...
   pixelsRendered = 0;
   pixelsToRender = w->render_ptr->vp.hres * w->render_ptr->vp.vres;
   
   //set the background
   wxBitmap bitmap(w->render_ptr->vp.hres, w->render_ptr->vp.vres, -1);
   wxMemoryDC dc;
   ...
   for(int x = 0; x < w->render_ptr->vp.hres; x += 16)
   {
      for(int y = 0; y < w->render_ptr->vp.vres; y += 16)
